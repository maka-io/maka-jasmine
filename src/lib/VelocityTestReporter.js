...
export default function VelocityTestReporter(options) {
  var self = this,
    timer = options.timer || noopTimer,
    ddpConnection = options.ddpConnection,
    ancestors = [],
    _jasmineDone
  ...
  var saveTestResult = Meteor.bindEnvironment(function saveTestResult(test) {
    ...
    if (test.failedExpectations[0]){
      var stack = removeStackTraceClutter(parseStack({stack: filterStack(test.failedExpectations[0].stack)}))
    ...
    ddpConnection.call('velocity/reports/submit', result, function (error){
      if (error){
  ...
  if (Meteor.isClient) {
    ...
    _jasmineDone = Meteor.bindEnvironment(function jasmineDone() {
      ddpConnection.call('velocity/reports/completed', {framework: options.framework});
      if (options.onComplete) {
        options.onComplete()
      }
    }, function (error) {
